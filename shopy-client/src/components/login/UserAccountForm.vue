     ...
     <form action="#" class="form " >
             ...
             <div class="title-center mb-6">or</div>
                        ...
                        <div class="form__group">
                            <input type="" class="form__input" v-model="accountEmail" placeholder="Email account" id="email" 
                                @input="$v.accountEmail.$touch()" @blur="$v.accountEmail.$touch()">
                                 <label for="email" class="form__label">Email account</label>
                             <div v-for="(i,ind) in emailErrors"  :key="ind" class="text-error mt-2">{{i}}</div>
                        </div>
                        <div class="form__group" >
                            <input type="password" class="form__input" v-model="accountPassword" placeholder="Password" id="password" 
                             @input="$v.accountPassword.$touch()" @blur="$v.accountPassword.$touch()">
                            <label for="password" class="form__label">Password</label>
                            <div v-for="(i,ind) in passwordErrors"  :key="ind" class="text-error mt-2">{{i}}</div>
                        </div>
                         <div class="form__group" >
                            <input type="password" class="form__input" v-model="repeatPassword" placeholder="Repeat password" id="passwordr" 
                             @input="$v.repeatPassword.$touch()" @blur="$v.repeatPassword.$touch()">
                            <label for="passwordr" class="form__label">Repeat password</label>
                            <div v-for="(i,ind) in repeatPasswordErrors"  :key="ind" class="text-error mt-2">{{i}}</div>
                        </div>
...
export default class UserAccountForm extends Vue{
    ...
    get passwordErrors(){
            ...
            !this.$v.accountPassword.minLength && errors.push("Password must have at least 6 letters."); 
