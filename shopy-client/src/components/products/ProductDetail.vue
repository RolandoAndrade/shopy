                 ...
                 <v-dialog v-model="commentDialog" scrollable max-width="600px">
                    <v-card>
                        ...
                        <v-card-text style="height: 300px;" v-if="comments.length>0">
                                    <div class="card-comment" v-for="(i,ind) in comments" :key="ind">
                                        <Icon size='icon-medium' icon='icon-bubbles2' color='orange-i' style="margin-right:30px;"/>
                        ...
                        <v-card-text v-else class="flex">
                                <div class="title-terciary mt-6">This publication has no comments</div>
                        </v-card-text>
                        <v-divider></v-divider>
                        <v-card-actions>
                        <v-btn color="purple" text @click="commentDialog = false">Close</v-btn>
                        </v-card-actions>
                    </v-card>
                 </v-dialog>      
    </v-container>
...
export default class ProductDetail extends Vue {
    ...
    private commentDialog : boolean = false;
