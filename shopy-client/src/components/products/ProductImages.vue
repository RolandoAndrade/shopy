<template>
    <div class="product-images flex column">
        <div class="product__image-container">
         ...
         <v-slide-group v-if="type === 'detail'" class="image-slider" center-active show-arrows color="purple">
            <v-slide-item v-for="(i, k) in product.productImages" :key="k" class="image-slider__img">
                <v-img contain :src="i.image" class="ma-2" @click="() => changePrimaryImg(k)" />
            </v-slide-item>
        </v-slide-group>
        <v-slide-group v-else class="image-slider" center-active show-arrows color="purple">
...
export default class ProductImages extends mixins(UpdateImagesLoader) {
    ...
    get image() {
        try {
            if (this.type === 'detail')  return this.product.productImages![this.indexToShow].image;
            else  return this.images[this.indexToShow].image;
        } catch (e) {
            return '';
        }
    }
