                ...
                ></v-select>
                 <v-container fluid v-if="active">
                        <v-select  
                            ...
                            v-model="value"
                            :items="subitems"
                            :hint="`${subitems.name}, ${subitems.id},${subitems.active}`"
                            item-text="name"
    ...
    private subCategories:Array<{category:string,active:boolean,childrens:Array<{id:number,name:string,active:boolean}>}> =[
        {
            category:'Technology',
            active:false,
            childrens:[{id:1,name:'Phones',active:false},{id:3,name:'Tv',active:false},{id:4,name:'Computers',active:false}]
        },
         {
            category:'Sports',
            active:false,
            childrens:[{id:5,name:'Soccer',active:false},{id:6,name:'Tennis',active:false},{id:7,name:'Softball',active:false},{id:8,name:'Baseball',active:false},{id:9,name:'Basketball',active:false},{id:10,name:'Karate',active:false}]
        }
    ]
    ...
    private selectItem(a: string){
        ...
        for (let i=0;i<this.subCategories.length;i++){
            if (this.subCategories[i].category === a){
               ...
               for(let j=0;j<this.subCategories[i].childrens.length;j++){
                   this.subitems.push({name:this.subCategories[i].childrens[j].name,
