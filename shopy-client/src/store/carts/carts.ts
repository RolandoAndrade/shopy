...
const carts: Module<CartsStateInterfaces, any> = {
    ...
    actions: {
        async [FETCH_CART]({ commit }, user: User): Promise<boolean> {
            try {
                const carts: Cart[] = await cartRepository.getAllFiltered(
                    new Filter({ userId: user.id })
                );
        ...
        async [DELETE_CART]({ commit, state }, cart: Cart) {
            try {
                ...
                if (d) {
                    commit(
                        SET_CART,
                        state.cart.filter(i => i.id !== cart.id)
                    );
                }
            } catch (e) {
                return false;
