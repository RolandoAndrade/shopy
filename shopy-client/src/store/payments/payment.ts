...
const payment: Module<PaymentStateInterface, any> = {
    ...
    getters: {
        ...
        {
            return (state.commission.processorFee||0) + (state.commission.serviceFee||0);
        }
    },
    mutations: {
    ...
    actions: {
        ...
        async [PAYMENTS_PAY](state, checkoutCarts: CheckoutCartsInterface): Promise<NewOrderInterface>
        {
            try {
                const response: NewOrderInterface = (await paymentRepository.post(checkoutCarts)).order;
