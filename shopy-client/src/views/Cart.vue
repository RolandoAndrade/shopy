        ...
        <v-row class="space-around to-column mt-10">
            ...
            <div class="column" style="width:max-content;">
                ...
                <div class="title-terciary ">
                    {{ this.$language.get('cart.total') }}: ${{ totalPrice }}
                </div>
            </div>
        </v-row>
        ...
        <div v-if="this.myCart && this.myCart.length">
            ...
            >
                <ButtonSecondary class="flex mr-8" @click.native="payAll">{{
                    this.$language.get('cart.btn-buy-all')
                }}</ButtonSecondary>
                <ButtonSecondary
                    class="flex"
                    v-if="selected.length > 0"
                    @click.native="paySelected"
                >
                    {{ this.$language.get('cart.buy') }} {{ selected.length }}
                    {{ this.$language.get('cart.items') }}</ButtonSecondary
                >
            </div>
        </div>
...
export default class CartView extends Vue {
    ...
    get totalPrice(): number {
        ...
        this.selectedItems.forEach(i => {
            sum +=
                i!.quantity! * (i.product!.price! - (i.product!.discount || 0));
        });
        return sum;
    }
