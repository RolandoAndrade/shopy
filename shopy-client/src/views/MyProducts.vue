        ...
        <div class="product-detail-container column to-cover-tablet">
            ...
            <div class="divider" style="height:1px; width:70%"></div>
             <div class="flex wrap cover" style="width:100%;">
                <ProductCard v-for="(n,k) in this.visibleProducts" :key="k" :product="n" >
                        <div>
         ...
         <v-dialog v-model="modalEdit" scrollable max-width="600px" >
                    <v-card v-if="currentProduct">
                        ...
                        <v-card-text style="height: 300px;padding-top:20px;" >
                                    ...
                                    <input v-model="currentProduct.title" type="" class="form__input mt-6 mb-10 " style="width:100%;" :placeholder="currentProduct.title" id="name" required>
                                    <Title size='title-secondary'>Description:</Title>
                                    <input v-model="currentProduct.description" type="" class="form__input mb-10" style="width:100%;" :placeholder="currentProduct.description" id="name" required>

                                    <div class="flex mb-10 mt-6">
                                        <Title size='title-secondary' class="mr-4">Dimentions:</Title>
                                        <input v-model="currentProduct.width" type="" class="form__input mr-2"  :placeholder="currentProduct.width" id="name" required>
                                                        x
                                        <input v-model="currentProduct.height" type="" class="form__input ml-2"  :placeholder="currentProduct.height" id="name" required>
                                    </div>
                                    <div class="flex mb-10 mt-6">                       
                                            <Title size='title-secondary' class="mr-4">Price:</Title>
                                            <div class="flex">
                                                <input v-model="currentProduct.price" type="" class="form__input mr-4"  :placeholder="currentProduct.price" id="name" required>    
                                            </div>
                                            <Title size='title-secondary'>Quantity:</Title>
                                            <div class="flex">
                                                <input v-model="currentProduct.stock" type="" class="form__input mr-4"  :placeholder="currentProduct.stock" id="name" required>    
                                            </div>                                        
                                    </div>
                                <div class="flex mb-4">
                                    <v-radio-group v-model="currentProduct.condition" row class="ml-4">
                        ...
                        <v-card-actions>
                            <div class="space-between">
                                <v-btn color="purple" text @click="modalEdit = false">Close</v-btn>
                                <v-btn color="purple" text @click="saveChanges">Save changes</v-btn>
                            </div>
                        </v-card-actions>
                    </v-card>
                </v-dialog>  
...
export default class MyProducts extends Vue {
    ...
    // mismos que en Select Info
        get numberErrors(){
            let error : string ='';
            if (!this.$v.currentProduct.price!.$invalid && !this.$v.currentProduct.width!.$invalid
                && !this.$v.currentProduct.height!.$invalid && !this.$v.currentProduct.stock!.$invalid)
                 return error;   
        ...
        get infoErrors(){
            ...
            if (!this.$v.currentProduct.price!.$invalid && !this.$v.currentProduct.width!.$invalid
                && !this.$v.currentProduct.height!.$invalid && !this.$v.currentProduct.stock!.$invalid
                 && !this.$v.currentProduct.title!.$invalid && !this.$v.currentProduct.description!.$invalid)
                        return errors;   
